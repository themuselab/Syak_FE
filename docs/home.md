...
> 상태: **UI 완료 + 백엔드 샵 연동 완료(`GET /shops`).** 지도만 placeholder(네이버 지도 = Phase B, dev build).
...
- **지도**: 현재 단색 placeholder(`MapPlaceholder`). Phase B에서 `NaverMapView`로 교체(`HomeMap`).
- **현재위치 버튼**: 지도 우하단 (Phase B에서 expo-location 연결).
...
```
- 마커 핀 PNG(`assets/icons/pin-*.png`)는 Phase B용(design.pen export, 색↔종류 확인 후).
...
## 5. 남은 작업
- **Phase B(네이버 지도, dev build)**: `@mj-studio/react-native-naver-map`+`expo-location` 설치, `app.config.ts` plugin(client_id)+Maven repo, `MapPlaceholder`→`HomeMap`(마커=`pin-*.png` 색별·현재위치·카메라), NCP 키, EAS 재빌드. (절차 [dev-build.md](./dev-build.md))
- **백엔드 갭(§4)** 반영 시 FE 연결(리뷰수·검색·categories·price_desc·시술/예약시간 필터).
- 무한스크롤(`total>limit`), favorite API 연동(로그인 후), 마커 탭=상세 이동 확정(`router.push('/shop/:id')`).

## 6. 검증
- `npm run typecheck`/`lint` 통과.
- web(`expo start --web`) + 로컬 BE(`docker compose up`): 목록·필터(districts/price_tiers/sort/has_event/has_slot)·검색·price_desc·빈상태/에러 확인. 지도는 placeholder. (categories는 백엔드 버그로 0건)
- 지도(Phase B): dev build에서 핀·3색 마커·핀탭 상세이동·현재위치.
