...
- **바텀시트**: 목록 모드 **3단 스냅 — 96px(최소) ↔ 40%(기본) ↔ 검색바 아래(최대)**. 칩바(고정) + 매장 목록 / 미리보기(핀 선택 시 카드 1개) / 로딩 / 에러 / 빈 상태. 목록 하단·필터 닫기 버튼은 **safe area bottom inset 반영**(안드 내비 바에 안 가림).
  ...
  - **제스처 임계값(2026-07-28 QA 3차)**: `activeOffsetY={[-10,10]}` + `failOffsetX={[-5,5]}`. 미지정이면 gorhom이 offset 제약을 아예 안 걸어 **가로 드래그 첫 픽셀에도 시트 pan이 활성**되고, 시트 로직은 `translationY`만 쓰므로 필터 칩바 가로 스크롤이 죽는다(상세 이미지 캐러셀도 동일). 세로 10px 이상일 때만 시트가 반응한다.
...
- **내 위치 버튼 위치(2026-07-18, 사용자 피드백 — 폰에서 시트와 겹침)**: ~~`useWindowDimensions` 높이 × 0.42 + 12~~ → **루트 컨테이너 onLayout 실측 높이 × 0.4 + 16**(시트 snapPoint '40%'와 같은 기준이라 기기 무관 정합). useWindowDimensions는 안드 상태바 처리에 따라 시트 컨테이너와 어긋날 수 있었음. 시트를 올리면 버튼이 가려지는 건 확정 정책(#45 고정) 유지.
