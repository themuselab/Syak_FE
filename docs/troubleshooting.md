...
- 해결: 실데이터 gu 고유값 89개를 전수 추출해 `src/shared/lib/region.ts`로 하드코딩 — 칩 label은 구/시, 전송 value는 원값. (경기 "광주시"와 광역시 "광주 동구"가 다른 지역인 것도 이 방식이라 안전)
- 관련: `region.ts`, `RegionFilterContent.tsx`, [decisions.md](./decisions.md)
- 교훈: 필터 값은 화면 라벨이 아니라 **DB 실값 기준**으로 계약을 확인할 것. 값 형식이 지역마다 다를 수 있다.
