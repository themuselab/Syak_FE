...
- 이유: 라벨 하드코딩은 데이터 현실과 어긋나 사용자 신뢰 문제(2026-07-04 "라벨 유지" 확정을 QA 근거로 뒤집음 — 당시엔 실데이터 규모 미파악). 전화번호는 tel: 변환 없이는 버튼이 무동작. 크롭은 렌더 값 조정(에셋별 캔버스 여백이 달라 코드 분기 필요)보다 단순.
- 대안(버림): 라벨 "예약하기" 단일화(네이버 샵에서 정보 손실), BE 데이터 정리만 대기(전화번호 25곳은 필드 성격상 정리 불확실 — FE 방어 필요), 핀 렌더 크기를 에셋별 분기(캔버스 여백이 제각각이라 상수 3벌).
- 관련: `ReservationBar.tsx`, `assets/icons/pin-*.png`, [shop-detail.md](./shop-detail.md), [home.md](./home.md)

## 2026-07-10 · 결정: 계정 관리 — 닉네임 PATCH 선제 구현(BE 미배포) + 탈퇴 후 로그인 이동
- 맥락/문제: 회원탈퇴(스토어 정책 필수)·계정관리·닉네임 디자인 도착(ZEDJC/o8fLU1/k0yZdi). GET·DELETE /users/me는 있으나 **닉네임 수정 API가 백엔드에 없음**. 닉네임은 소셜 프로필 원천이라 null 가능(카카오 미동의·네이버 name/nickname 모두 미제공) — "설정하기" 표시가 그 대응.
- 결정(사용자 확정): ① 닉네임 화면을 **UI+API 호출까지 선제 구현** — 계약 `PATCH /users/me {nickname} → 200 UserProfile`로 FE 완성, BE 배포 즉시 동작(그전까지 404 → 실패 문구). BE 요청 전달문 account.md §4 ② 탈퇴 성공 후 **로그인 화면 replace**(BE 체크리스트와 일치) ③ 비회원은 마이의 계정 관리 버튼 숨김 + 화면 게이팅 병행 ④ 탈퇴 세션 정리는 useSignOut 패턴 재사용하되 **성공 시에만** 정리(실패 시 세션 유지 — 재시도 가능) ⑤ RHF 첫 도입(닉네임 폼 — CLAUDE.md 규칙).
- 이유: BE 추가가 단순(스펙 제안 첨부)해 FE 선제 구현이 재작업 없이 이어짐. 탈퇴는 로그아웃과 달리 실패 시 로컬만 정리하면 "계정은 남았는데 앱은 로그아웃" 불일치가 생겨 성공 시에만 정리.
- 대안(버림): 닉네임 read-only(디자인 흐름 위반 — 설정하기 탭 목적지 없음), BE 배포 대기 후 일괄 구현(출시 필수인 탈퇴가 같이 늦어짐), 탈퇴 후 홈 비회원 모드(로그아웃 동선과 불일치).
- 관련: `src/screens/account/*`, `user.api.ts`/`user.queries.ts`, [account.md](./account.md)
